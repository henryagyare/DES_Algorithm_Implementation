class DESOperations:
    def initialPermutation(self, data):
        # print(f"data = {data}")
        ...
        for i in ip_table:
            for j in i:
                ...
                zerosList[position] = str(data[j - 1])
                # print(f"zerosList[{j}] = {zerosList[j - 1]}")
                position += 1
        
        initPermValue = "".join(zerosList)
        return initPermValue


    def expansion(self, expansion_data):
        expansion_table = [
            [32, 1, 2, 3, 4, 5], 
            [4, 5, 6, 7, 8, 9], 
            [8, 9, 10, 11, 12, 13], 
        ...
        for i in expansion_table_content: 
            # print(i)
            zerosList[position] = str(expansion_data[i - 1])
            position += 1
    ...
    def substitution(self, subVal):
        ...
        while i < len(S_Boxes):
            ...
            i += 1
        # print(f"chunksArray = {chunksArray}")
            

        def substitute(subBox, subData: str):
            ...
            # print(f"substituted_binary {substituted_binary}")
            
            return substituted_binary

        postSubstitution = []
        for i in range(len(chunksArray)):
            postSubstitution.append(substitute(S_Boxes[i], chunksArray[i]))

        postSubstitution = "".join(postSubstitution)

        return postSubstitution


    def permutation(self, permVal):
        permVal = str(permVal)
        P = [
            [16, 7, 20, 21], 
            [29, 12, 28, 17], 
...
class KeyGenOperations:
    def permutationChoice1(self, key_data):
        PC1_table = [
            [57, 49, 41, 33, 25, 17, 9], 
            [1, 58, 50, 42, 34, 26, 18], 
            [10, 2, 59, 51, 43, 35, 27], 
        ...
        for i in PC1_table:
            for j in i:
                # print(j)
                zerosList[position] = key_data[j - 1]
        ...
        PC1_val = "".join(str(bit) for bit in new_zerosList)
        return PC1_val
    

    def leftCircularShift(self, half_key, shift_amount):
        half_key = str(half_key)

        # Calculate the actual shift amount considering circular shift
        shift_amount %= len(half_key)
        
        # Perform left circular shift using slicing
        shifted_string = half_key[shift_amount:] + half_key[:shift_amount]
        
        return shifted_string
    

    def permutationChoice2(self, key_value):
        PC2_table = [
            [14, 17, 11, 24, 1, 5], 
            [3, 28, 15, 6, 21, 10], 
            [23, 19, 12, 4, 26, 8], 
        ...
        for i in PC2_content:
            zerosList[position] = str(key_value[i - 1])
            position += 1
        ...
        PC2_val = "".join(str(bit) for bit in new_zerosList)

        return PC2_val
    

class Conversions:
    def textTo64bit(self, text):
        binary_string = ""
        for char in text:
